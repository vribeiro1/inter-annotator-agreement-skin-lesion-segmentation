...
class GaussCRF(nn.Module):
    ...
    def __init__(self, shape=(256, 256), conf=default_conf, n_classes=None):
        ...
        if conf["weight"] is None:
            ...
            val = conf["weight_init"]
            weight = val * torch.ones(1, n_classes, 1, 1)

        self.CRF = ConvCRF(
            shape, n_classes, mode="col", conf=conf, filter_size=conf["filter_size"],
...
class MessagePassingCol:
    ...
    def __init__(self, feat_list, compat_list, merge, npixels, n_classes, norm="sym", filter_size=5, clip_edges=0,
                 blur=1, matmul=False, pyinn=False):
        if not norm == "sym" and not norm is None:
        ...
        if not torch.cuda.is_available():
            print("WARNING: CUDA is not available. Weird behavior may happen")
    ...
    def _get_norm(self, gaus):
        ...
        normalization_feats = normalization_feats.to(self.device)
...
class ConvCRF(nn.Module):
    ...
    def __init__(self, npixels, n_classes, conf, mode="conv", filter_size=5,
                 clip_edges=0, blur=1, norm="sym", merge=False, trainable=False,
                 convcomp=False, weight=None, final_softmax=True, unary_weight=10, pyinn=False):
        ...
        if convcomp:
            self.comp = nn.Conv2d(n_classes, n_classes,
                                  kernel_size=1, stride=1, padding=0,
                                  bias=False)

            self.comp.weight.data.fill_(0.1 * math.sqrt(2.0 / n_classes))
        else:
            self.comp = None
